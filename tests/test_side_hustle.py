...
def test_read_payments(client, auth_headers):
    client.post(
        "/api/v1/side-hustle/payments",
    ...
    response = client.get("/api/v1/side-hustle/payments", headers=auth_headers)
...
def test_delete_payment(client, auth_headers):
    ...
    response = client.delete(f"/api/v1/side-hustle/payments/{payment_id}", headers=auth_headers)
    assert response.status_code == 204
    
    get_response = client.get("/api/v1/side-hustle/payments", headers=auth_headers)
    assert len(get_response.json()) == 0

def test_net_cash(client, auth_headers):
    client.post(
        "/api/v1/side-hustle/payments",
    ...
    assert response.status_code == 200
    assert response.json()["net_side_hustle"] == 150.0
